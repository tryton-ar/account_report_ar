...
class CitiWizard(Wizard):
    ...
    def default_exportar(self, fields):
        logger.info('exportar CITI RG3685')
    ...
    def export_citi_alicuota_ventas(self):
        logger.info('exportar CITI RG3685 Alicuota Ventas')
        pool = Pool()
        Currency = pool.get('currency.currency')

        lines = ""
        invoices = self._get_invoices_sale()
    ...
    def export_citi_comprobante_ventas(self):
        logger.info('exportar CITI RG3685 Comprobante Ventas')
        pool = Pool()
        Currency = pool.get('currency.currency')
    ...
    def _get_invoices_purchase(self):
        pool = Pool()
        Invoice = pool.get('account.invoice')
    ...
    def export_citi_alicuota_compras(self):
        logger.info('exportar CITI RG3685 Comprobante Compras')
        pool = Pool()
        Currency = pool.get('currency.currency')

        lines = ""
        invoices = self._get_invoices_purchase()
        for invoice in invoices:
