...
class Subdiario(object):
    ...
    @classmethod
    def get_invoices_purchase(cls, data):
        pool = Pool()
    ...
    @classmethod
    def get_invoices_sale(cls, data):
        pool = Pool()
        Invoice = pool.get('account.invoice')
...
class SubdiarioPurchase(Wizard):
    ...
    start = StateView('subdiario.purchase.start',
        'account_report_ar.purchase_start_view_form', [
...
class SubdiarioPurchaseReport(Report, Subdiario):
    ...
    def get_context(cls, records, header, data):
        ...
        invoices = cls.get_invoices_purchase(data)
        for invoice in invoices:
...
class SubdiarioSale(Wizard):
    ...
    start = StateView('subdiario.sale.start',
        'account_report_ar.sale_start_view_form', [
            Button('Cancel', 'end', 'tryton-cancel'),
            Button('Print Spreadsheet', 'print_', 'tryton-print', True),
            Button('Print PDF', 'print_pdf', 'tryton-print'),
            ])
...
class SubdiarioSaleReport(Report, Subdiario):
    ...
    def get_context(cls, records, header, data):
        ...
        invoices = cls.get_invoices_sale(data)
        for invoice in invoices:
            total_op_exentas += cls.get_exento(invoice, invoice.lines)
            total_neto_gravado += cls.get_gravado(invoice, invoice.lines)
            total_conc_no_gravados += cls.get_no_gravado(invoice, invoice.lines)
